...
tmux new-session -d -s "${ENV_SESSION}" \
  "cd ${ROOT} && ${CONDA_SH:+CONDA_SH=${CONDA_SH}} NUM_ENVS=${NUM_ENVS} BASE_PORT=${BASE_PORT} bash ${ROOT}/scripts/run_sciworld_env_service.sh"
...
FWD=""
for v in CONDA_SH TRAIN_ENV TRAIN_FILE HF_HOME MAX_ROUNDS PPO_MINI_BATCH_SIZE AF_LORA_ENABLE AF_LORA_RANK AF_LORA_ALPHA AF_LORA_LR AF_LORA_TARGETS VLLM_PORT ENABLE_ERC ERC_CLIPPING_METHOD ERC_CLIPPING_TYPE ERC_MOMENTUM UNCERTAINTY_SCALE_KAPPA UNCERTAINTY_SCALE_MIN UNCERTAINTY_SCALE_RENORMALIZE SAFE_COMMIT_MODE SAFE_COMMIT_OMEGA SAFE_COMMIT_RECENCY SAFE_COMMIT_RECENCY_GAMMA SAFE_COMMIT_SUCCESS_THRESHOLD SAFE_COMMIT_KAPPA SAFE_COMMIT_GMAX SAFE_COMMIT_GMIN SAFE_COMMIT_RENORMALIZE SAFE_COMMIT_TEXT_GATE SAFE_COMMIT_GATE_MODE SAFE_COMMIT_CLF_ENV SAFE_COMMIT_CLF_WINS_ONLY SAFE_COMMIT_CLF_MAX_NEW \
         ...
         SFT_ABLATION_ENABLE SFT_ABLATION_COEF SFT_ABLATION_GATE SFT_ABLATION_SUCCESS_THRESHOLD \
         SFT_ABLATION_MAX_LENGTH SFT_ABLATION_TARGET SFT_ABLATION_MAX_SAMPLES_PER_TRAJ \
         SFT_MICRO_BATCH_SIZE_PER_GPU PPO_MICRO_BATCH_SIZE_PER_GPU \
