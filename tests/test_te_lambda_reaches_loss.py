...
SRC = 'AgentGym-RL/verl/workers/agent_actor/dp_actor.py'
...
# --- 3) 循环体内的 _te_lam 必须来自顶层变量，不能再碰 data.meta_info ---
...
for node in ast.walk(loop):
    if isinstance(node, ast.Assign):
        for t in node.targets:
            if isinstance(t, ast.Name) and t.id == '_te_lam':
                found += 1
                assert isinstance(node.value, ast.Name) and node.value.id == 'te_lambda', \
                    ("循环体内的 _te_lam 必须直接取顶层 te_lambda；"
                     f"实际是 {ast.dump(node.value)[:80]} —— 那里没有 meta_info")
assert found == 1, f"循环体内应恰好有一处 _te_lam 赋值，实际 {found} 处"
...
# --- 5) TE-KL 门控仍是双重的 ---
assert "_te_lam > 0.0 and 'te_log_q' in data.keys()" in src_txt, "TE-KL 双重门控被破坏"
